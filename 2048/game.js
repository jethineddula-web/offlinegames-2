...
(function () {
  ...
  function watchAdToUndo() {
    // Formerly gated behind an interstitial. Rewarding a user for viewing an
    // ad breaches AdSense policy, so the undo is granted outright.
    if (!G.lastAlive) return;
    playClick();
    revive();
  }
    ...
    if (showOver) {
      var over = el(
        '<div class="overlay" role="dialog"><div class="overlay-card">' +
          ...
          (G.canRevive ? " · you can undo the last slide" : "") +
      ...
      if (G.canRevive) {
        ...
        adBtn.textContent = "Undo last slide";
    ...
    if (G.help) {
      var help = el(
        '<div class="overlay" role="dialog" data-no-swipe><div class="overlay-card">' +
          ...
          "<li>Out of moves? You can undo the last slide. Best score is saved on this device.</li>" +
          "</ol></div></div>"
      );
